...
legend(off) ///
by(, legend(off)) ///
by(, note("Source: {fontface Lato:Author's analysis of IPUMS-CPS.} Sample: {fontface Lato:U.S. noninstitutional civilians age 18 or older.}" `notes' "@johcharts on Substack", margin(l+1.5) color(gs7) span size(vsmall) position(7))) ///
